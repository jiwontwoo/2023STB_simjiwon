...
foodggd <- read.csv("ggd_food_2023.csv", na="", stringsAsFactors = F)
...
#날짜데이터를 분석용 데이터로 변경
#1.YYYYMMDD형식으로 변경
foodggd$open_date <- gsub("-","",foodggd$open_date)
foodggd$close_date <- gsub("-","",foodggd$close_date)
#2.문자형 데이터를 정수형 데이터로 변환
foodggd$open_date <- as.integer(foodggd$open_date)
foodggd$close_date <- as.integer(foodggd$close_date)
#3.변경된 데이터구조 확인
str(ggdfood)
...
foodggd <- foodggd %>% 
  ...
  select(name,type,status,open_date,close_date,address)
#처리결과 확인
table(foodggd$status)

#2.type변수
table(foodggd$type)
#업종에 컬럼의 모든 값이 포함된 레코드 제외외
#foodggd <- foodggd %>% filter(address != '서울특별시 동대문구 용두동 235-18' | open_date != '20190708' | close_date != '20210902') %>%select(name,type,status,open_date,close_date,address)

#3.open_date변수
range(foodggd$open_date, na.rm = T) 
table(is.na(foodggd$open_date))#결측치 없음
foodggd$open_year<-substr(foodggd$open_date,1,4)#인허가년도 변수 생성

#4.close_date변수
range(foodggd$close_date, na.rm = T)
foodggd$close_year<-substr(foodggd$close_date,1,4)#인허가년도 변수 생성

#5.address변수
foodggd$district<-substr(foodggd$address,5,7)#구 정보를 분리하여 변수 생성
table(foodggd$district)#이상치 확인
foodggd$district <- ifelse(foodggd$district%in%c("번지","회","시 관","시 강","시 계","시 금","시 남","시 노","시 마","시 미","시 서","시 용","시 은","106"),NA,foodggd$district)#이상치제거
table(foodggd$district)#이상치 확인
...
#문자형데이터를 정수형으로 변경
foodggd$open_year <- as.integer(foodggd$open_year)
foodggd$close_year <- as.integer(foodggd$close_year)
str(foodggd)
