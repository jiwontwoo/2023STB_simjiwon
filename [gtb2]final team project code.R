...
#.open_date null 제외
foodshop <- foodshop %>% 
  filter(name != '대연할매국밥집' | type != '경양식' ) %>%
  select(name,type,status,open_date,close_date,address)

foodshop <- foodshop %>% 
  filter(name != '본동집' | address != '부산광역시 동래구 온천동 1134-1번지' ) %>%
  select(name,type,status,open_date,close_date,address)
...
foodshop$district<-substr(foodshop$address,7,8)#군/구 정보를 분리하여 변수 생성
table(foodshop$district)#이상치 확인
foodshop$district <- ifelse(foodshop$district%in%c(" 2"," 6","번지","지"),NA,foodshop$district)#이상치제거
table(foodshop$district)#이상치 확인

foodshop <- foodshop %>% 
  filter(name != '오뚜기분식' | open_date != '19841008' ) %>%
  select(name,type,status,open_date,close_date,address,open_year,close_year,district)

foodshop <- foodshop %>% 
  filter(name != '왕상횟집' | open_date != '19930531' ) %>%
  select(name,type,status,open_date,close_date,address,open_year,close_year,district)

foodshop <- foodshop %>% 
  filter(name != '서울횟집' | open_date != '19920612' ) %>%
  select(name,type,status,open_date,close_date,address,open_year,close_year,district)

foodshop <- foodshop %>% 
  filter(name != '대변활어회센타' | open_date != '19930508' ) %>%
  select(name,type,status,open_date,close_date,address,open_year,close_year,district)

foodshop <- foodshop %>% 
  filter(name != '구구분식' | open_date != '19930508' ) %>%
  select(name,type,status,open_date,close_date,address,open_year,close_year,district)
